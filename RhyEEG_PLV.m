function [plv] = RhyEEG_PLV(eegData, filtSpec, dataSelectArr)
...
% filtSpec.order = 10000;
% filtSpec.range = [0 20];
srate = 5000;
%eegData = eegData(:,:,1:20);
%phase2 = eegData(:,:,21:40);
% Input parameters:
...
for k = 1:32
    ...
    if k ~= 23
        if k <23
            plot((0:size(eegData, 2)-1)/srate, squeeze(plv(:, k, 23, :)));
            xlabel('Time (s)');
            ylabel('Plase Locking Value');
            title(['PLV between CZ and ' num2str(name)])
            ax = gca;
            set(ax,'XTick',[0 .5 .75 1 1.25 2 4 6 8])
            grid
        end
        if k >23
            plot((0:size(eegData, 2)-1)/srate, squeeze(plv(:, 23, k, :)));
            xlabel('Time (s)');
            ylabel('Plase Locking Value');
            title(['PLV between CZ and ' num2str(name)])
            ax = gca;
