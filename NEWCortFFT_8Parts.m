function [ EvokedFFTdata] =NEWCortFFT_8Parts( ArtifactedEpochData, Fs, subID, s, Condition, c, TapKey)
...
if c ==1 || c==2
    ...
    if isempty(negPhase) || isempty(posPhase)
        ...
        evokeddata{7} = data(:,:,37:42);
        evokeddata{8} = data(:,:,43:48);
...
%     subplot(2,2,(i-33))
%     name=chnames{i};
%     MaxAmp = max(2*abs(CorticalFFTdata(8:index6Hz,i)));
