...
% 1. June 2015: Modified by eeskoe. The original name  of the EEG file (i.e., how it was first called in the recording software) is stored in
% the vhdr file. If the set of filed (vmrk, vhdr, eeg) is renamed after
% data collection, pop_loadbv was expecting the original file name. The
% code has been updated such that it will look for an eeg file that has the
% same base name as the vhdr file. 
...
% next two lines added by eeskoe in June 2015
[p name ext] = fileparts(hdrfile);
hdr.commoninfos.datafile = [name, '.eeg'];
...
if isfield(hdr.commoninfos, 'markerfile')
    ...
    %next two lines added by eeskoe June 15
    [p name ext] = fileparts(hdrfile);
    hdr.commoninfos.markerfile = [name, '.vmrk'];
