function [topchans] = FULLCorticalFFT_Avg3PLOT_V2( CorticalFFTdata, Fs )
...
top5=nan(3,size(CorticalFFTdata,3));
figure
hold on
...
%     goodChanCount = 1;
    ...
    for e = 1:32
        ...
        NormedFFTData(:,e,s)= (CorticalFFTdata(:,e,s)./MaxAmp);
        %         [peaks,locs] = findpeaks((2*abs(NormedFFTData(index025Hz:index6Hz,e,s))),'SortStr','descend');
...
%     goodChans
%     [sortedX,sortingIndices] = sort((2*abs(NormedFFTData(index2Hz,goodChans,s))),'descend');
    [sortedX,sortingIndices] = sort((2*abs(NormedFFTData(index2Hz,1:32,s))),'descend');
    
    if size(sortingIndices)<3
        ...
        top5(:,s) = sortingIndices(1:3);
        plotdata(:,1:3,s) = NormedFFTData(:,top5(:,s),s);
        sortedX(1:3)
    end
    
    BestEdata(:,s) = nanmean(plotdata(:,:,s),2);
    plot(f,(2*abs(BestEdata(1:NFFT/2+1,s))),'linewidth', 1, 'color', [.5 .5 .5]);
...
%Finishing Touches
title('FFT of Responses at 3 Best Electrodes per Subject')
xlabel('Frequency (Hz)')
