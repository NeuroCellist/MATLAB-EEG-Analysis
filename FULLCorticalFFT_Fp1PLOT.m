function [] = FULLCorticalFFT_Fp1PLOT( CorticalFFTdata, Fs )
...
for s = 1:size(CorticalFFTdata,3)  
        MaxAmp = max(2*abs(CorticalFFTdata(8:192,1,s)));
        NormedFFTData= (CorticalFFTdata/MaxAmp);
        plot(f,(2*abs(NormedFFTData(1:NFFT/2+1,1,s))),'linewidth', 1, 'color', [.5 .5 .5]);    
end
...
MaxAmp1 = max(2*abs(GroupCorticalFFTdata(8:192,1)));
NormedFFTData1= (GroupCorticalFFTdata/MaxAmp1);
GrpAvgPlot=(2*abs(NormedFFTData1(1:NFFT/2+1,1)));
plot(f,GrpAvgPlot,'b');
%Finishing Touches
title('FFT of Responses at Fp1')
xlabel('Frequency (Hz)')
