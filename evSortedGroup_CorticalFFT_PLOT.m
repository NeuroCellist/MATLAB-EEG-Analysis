function [] = evSortedGroup_CorticalFFT_PLOT( gooddata, baddata, Fs )
...
chan = size(gooddata,2);

T = 1/Fs;                     % Sample time
L = length(gooddata/Fs);   % Length of signal
t = (0:L-1)*T;                % Time vector
NFFT = L*4; % Next power of 2 from length of y

goodavgdata = nanmean(gooddata,3);
badavgdata = nanmean(baddata,3);
goodFFTdata= fft(goodavgdata,NFFT)/L;
badFFTdata= fft(badavgdata,NFFT)/L;
...
for k = 1:chan
    ...
    badplot = detrend(2*abs(bad_NormedFFTData(1:NFFT/2+1,k)));
    goodplot = detrend(2*abs(NormedFFTData(1:NFFT/2+1,k)));
