function [] = FULLCorticalFFT_CzPLOT( CorticalFFTdata, Fs )
...
for s = 1:size(CorticalFFTdata,3)  
        MaxAmp = max(2*abs(CorticalFFTdata(8:192,23,s)));
        NormedFFTData= (CorticalFFTdata/MaxAmp);
        plot(f,(2*abs(NormedFFTData(1:NFFT/2+1,23,s))),'linewidth', 1, 'color', [.5 .5 .5]);    
end
%Plots the Stim Envelope FFT
MaxAmp2 = max(2*abs(GroupCorticalFFTdata(8:192,32)));
NormedFFTData2= (GroupCorticalFFTdata/MaxAmp2);
plot(f,(2*abs(NormedFFTData2(1:NFFT/2+1,32))), 'r');
% Plots the Group Averaged FFT
MaxAmp1 = max(2*abs(GroupCorticalFFTdata(8:192,23)));
NormedFFTData1= (GroupCorticalFFTdata/MaxAmp1);
GrpAvgPlot=(2*abs(NormedFFTData1(1:NFFT/2+1,23)));
plot(f,GrpAvgPlot,'b');
%Finishing Touches
title('FFT of Responses at Cz')
xlabel('Frequency (Hz)')
